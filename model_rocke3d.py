...
def calc_rocke3d_rei(
    ...
    """
    Calculate effective radius of ice crystals from ROCKE3D output.
    ...
    temp: xarray.DataArray
        ROCKE-3D data array of air temperature.

    These are the parameters used in the temperature dependent aggregate
    parameterizations for ice cloud particle sizes listed below.
    a0_agg_cold = 7.5094588E-04,
    b0_agg_cold = 5.0830326E-07,
    a0_agg_warm = 1.3505403E-04,
    b0_agg_warm = 2.6517429E-05,
    ...
    """
    # Calculate the R_eff
    rei = xr.where(temp < t_thresh)

    # FIXME: the next 2 lines do not assign output to anything
    a0_agg_cold * da.exp(s0_agg * (temp - t0_agg)) + b0_agg_cold
    a0_agg_warm * da.exp(s0_agg * (temp - t0_agg)) + b0_agg_warm
